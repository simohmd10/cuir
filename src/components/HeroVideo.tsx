...
export default function HeroVideo({ videoSrc, mp4Src, posterSrc }: HeroVideoProps) {
  ...
  return (
    <section
      dir={dir}
      className="relative w-full h-[100svh] md:h-[100vh] overflow-hidden flex items-center justify-center hero-grain"
    ...
    >
      ...
      <div className="relative z-10 w-full container-luxury flex flex-col items-center text-center px-6">
        ...
        <p className={[
          ...
          'tracking-wide leading-relaxed text-white/80',
          isAr
        ...
        <div className="hero-ctas mt-10 md:mt-14 flex flex-wrap items-center justify-center gap-6 md:gap-10">
          ...
          <Link to="/shop" className="group relative inline-flex items-center gap-2 pb-0.5">
            <span className={[
              ...
              'text-white text-xs md:text-[13px]',
              'transition-colors duration-500 ease-[var(--ease-luxury)]',
              'group-hover:text-camel',
            ].join(' ')}>
              {primaryCta}
              <span className="absolute -bottom-px left-0 right-0 h-px bg-white/80 group-hover:bg-camel transition-colors duration-500 ease-[var(--ease-luxury)]" />
            </span>
          </Link>
      ...
      >
        ...
        <ChevronDown
          className="w-4 h-4 text-white/65 hero-bounce"
          strokeWidth={1.5}
        />
      </div>
    </section>
